...
import type { Order, Product } from "@/sanity.types";
import Image from "next/image";
import { imageUrl } from "@/lib/imageUrl";

// Custom type for resolved order data where products contain actual product objects
type ResolvedOrder = Omit<Order, "products"> & {
  products?: Array<{
    product?: Product;
    quantity?: number;
    _type: "productItem";
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      <div className="bg-white p-4 sm:p-8 rounded-xl shadow-lg w-full max-w-4xl">
        <h1 className="text-4xl font-bold text-zinc-800 tracking-light mb-8">
          My Orders
        </h1>
        {orders.length === 0 ? (
          ...
          <div className="space-y-6 sm:space-y-8">
            {orders.map((order: ResolvedOrder) => (
              <div
                ...
                <div className="p-4 sm:p-6 border-b border-gray-300">
                  ...
                  <div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
                    <div className="flex items-center">
                      ...
                      <span
                        className={`px-3 py-1 rounded-full text-sm ${order.status === "completed" ? "bg-green-200" : "bg-zinc-200"}`}
                      >
                        {order.status}
                      </span>
                    </div>
                ...
                <div className="px-4 py-3 sm:px-6 sm:py-4">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    {order.products?.map((product) => (
                      <div
                        className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 py- border-b last:border-b-0"
